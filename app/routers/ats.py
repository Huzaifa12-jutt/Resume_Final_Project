...
async def upload_profile_resume(file: UploadFile = File(...), user=Depends(require_role('candidate'))):
    if not file.filename.lower().endswith('.pdf'): raise HTTPException(415,'Only PDF resumes are supported')
    content=await file.read()
    if len(content) > 10 * 1024 * 1024: raise HTTPException(413,'Resume exceeds 10MB limit')
    try: parsed=parse_resume(io.BytesIO(content),file.filename)
    except Exception as exc: raise HTTPException(422,f'Could not parse resume: {exc}')
    path=upload_resume_file('profiles',user['id'],content); db=get_supabase()
    ...
    existing=db.table('candidate_profiles').select('user_id').eq('user_id',user['id']).execute()
    return (db.table('candidate_profiles').update(data).eq('user_id',user['id']) if existing.data else db.table('candidate_profiles').insert(data)).execute().data[0]
...
def recruiter_jobs(user=Depends(require_role('recruiter'))):
    ...
    if jobs:
        ...
        # Count real candidates from the SAME table the job detail dashboard reads from,
        # so the count shown on the card always matches what you see after "Open Dashboard".
...
def list_applications(user=Depends(current_user)):
    db=get_supabase()
    if user['role']=='candidate': return db.table('applications').select('*').eq('candidate_id',user['id']).order('applied_at',desc=True).execute().data or []
    company=_company_for(user['id'])
...
def analytics(user=Depends(require_role('recruiter'))):
    ...
    # Fetch candidates + their nested scores ONCE, and reuse this single result set for
    # both the recent-candidates list AND the average score, instead of running a second,
    # separate full-table scores query for the same data (that duplicate select is removed).
    candidates = []
    if ids:
    ...
    for c in recent_candidates_source:
        ...
        job_title = job_match['title'] if job_match else 'Unknown Role'
