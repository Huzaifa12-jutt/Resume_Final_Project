...
export default function RecruiterMessagesPage() {
  useDocumentTitle('Candidate Messages — TEEROP');
  const { user } = useAuth();
  ...
  useEffect(() => {
    loadConversations().then((data) => {
      if (!conversationId && data.length > 0 && window.innerWidth >= 768) {
        navigate(`/recruiter/messages/${data[0].id}`, { replace: true });
      }
    });
  ...
  return (
    <RoleShell
      title="Messages"
      subtitle="Direct communication with applicants on your job listings."
      role="recruiter"
    >
